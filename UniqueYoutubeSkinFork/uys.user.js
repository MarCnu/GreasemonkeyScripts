...
GM_config.init('Unique Youtube Skin Settings', {
	hider : {
		...
		title : 'Scale videos full window size and hide sidebar until mouse scroll or arrow keys pressed',
		type : 'checkbox',
	default:
		true
	},
	cooler : {
		label : 'Cool Black Theme',
		title : 'Make background dark in very cool way',
		type : 'checkbox',
	default:
		true
	},
	...
	relogo : {
		label : 'Youtube Logo to Subscriptions',
		title : 'Redirect youtube logo to subscription uploads',
		type : 'checkbox',
	default:
		true
	},
	sbhider : {
		label : 'No Scrollbars',
		title : 'Hide scrollbar (only for chrome)',
		type : 'checkbox',
	default:
		true
	},
	expande2 : {
		...
		title : 'Collapse description and expand auto on mouseover',
		type : 'checkbox',
	default:
		true
	},
	collapse : {
		...
		title : 'Collapse comments and expand auto on mouseover',
		type : 'checkbox',
	default:
		true
	},
	collaps2 : {
		label : 'Collapse Related Videos',
		title : 'Collapse related videos and expand on mouseover',
		type : 'checkbox',
	default:
		true
	},
	vrhider : {
		label : 'Hide Video Responses',
		type : 'checkbox',
	default:
		true
	},
	...
	lbhider : {
		label : 'Hide Like Bar and View Count',
		type : 'checkbox',
	default:
		false
	},
	ybhider : {
		label : 'Hide Logo and Search Bar',
		type : 'checkbox',
	default:
		false
	},
	ybfixer : {
		label : 'Search Bar Always on Top',
		title : 'Make youtube logo and search bar allways visible',
		type : 'checkbox',
	default:
		false
	},
	nomouse : {
		label : 'Disable Hiding Sidebar via Mouse',
		title : 'No hiding sidebar and resizing video when mouse reach right or left edge of window',
		type : 'checkbox',
	default:
		false
	},
	...
	flash : {
		label : 'Force Flash Player (No Html5)',
		title : 'May cause a small delay loading flash player',
		type : 'checkbox',
	default:
		false
	},
...
(function (undefined) {
	...
	if (window !== window.top || !URL.match(rYoutubeUrl)/*|| URL.match(rYoutubeBlacklistedUrl)*/
	)
	{
		return;
	}
...
if (window.location.href.indexOf("youtube.com/watch") >= 0) {
	...
	if (GM_config.get("reverser")) {
		...
		$("#player, #player-legacy, .site-left-aligned.guide-enabled #player-legacy").css("width", "-moz-calc(100% - 312px)");
		$("#player, #player-legacy, .site-left-aligned.guide-enabled #player-legacy").css("width", "-webkit-calc(100% - 312px)");
		$("#player, #player-legacy, .site-left-aligned.guide-enabled #player-legacy").css("width", "-o-calc(100% - 312px)");
		$("#player, #player-legacy, .site-left-aligned.guide-enabled #player-legacy").css("width", "calc(100% - 312px)");
	...
	if (!GM_config.get("hider")) {
		$("#player").css("width", "-moz-calc(100% - 312px)");
		$("#player").css("width", "-webkit-calc(100% - 312px)");
		$("#player").css("width", "-o-calc(100% - 312px)");
		$("#player").css("width", "calc(100% - 312px)");
		$("#watch7-main-container, #yt-masthead-container, .watch7-playlist-bar-right, #watch7-playlist-tray-container, #watch7-playlist-tray, .watch7-playlist-bar-left, #watch7-playlist-tray-container").css("display", "block");
	}
	...
	function GRT_key(event) {
		...
		if (String.fromCharCode(event.which) == "Z" && !event.ctrlKey && !event.altKey && !event.metaKey) {
			if (GM_config.get("reverser")) {
				...
				$("#player, #player-legacy, .site-left-aligned.guide-enabled #player-legacy").css("padding-left", "0px");
				$("#player, #player-legacy, .site-left-aligned.guide-enabled #player-legacy").css("width", "100%");
				$("#lightsOut, #watch7-main-container, #yt-masthead-container, .watch7-playlist-bar-right, #watch7-playlist-tray-container, #watch7-playlist-tray, .watch7-playlist-bar-left, #watch7-playlist-tray-container").css("display", "none");
			} else {
				$("#player, #player-legacy, .site-left-aligned.guide-enabled #player-legacy").css("padding-left", "0px");
				$("#player, #player-legacy, .site-left-aligned.guide-enabled #player-legacy").css("padding-right", "0px");
				$("#player, #player-legacy, .site-left-aligned.guide-enabled #player-legacy").css("width", "100%");
		...
		if (String.fromCharCode(event.which) == "X" && !event.ctrlKey && !event.altKey && !event.metaKey) {
			if (GM_config.get("reverser")) {
				...
				$("#player, #player-legacy, .site-left-aligned.guide-enabled #player-legacy").css("width", "-o-calc(100% - 312px)");
				$("#player, #player-legacy, .site-left-aligned.guide-enabled #player-legacy").css("width", "calc(100% - 312px)");
				$("#player, #player-legacy, .site-left-aligned.guide-enabled #player-legacy").css("padding-left", "311px");
				$("#player, #player-legacy, .site-left-aligned.guide-enabled #player-legacy").css("padding-right", "1px");
				$(".watch7-playlist-bar-right, #watch7-playlist-tray-container, #watch7-playlist-tray, .watch7-playlist-bar-left, #watch7-playlist-tray-container, #watch7-main-container, #yt-masthead-container").css("display", "block");
			}
			try {
				event.preventDefault();
			} catch (e) {}
	...
	if (!GM_config.get("nomouse")) {
		...
		if (!GM_config.get("reverser")) {
			$(window).mousemove(function (e) {
				...
				if (now > past) {
					$("#watch7-main-container, #yt-masthead-container, .watch7-playlist-bar-right, #watch7-playlist-tray-container, #watch7-playlist-tray, .watch7-playlist-bar-left, #watch7-playlist-tray-container").css("display", "none");
					$("#player, #player-legacy, .site-left-aligned.guide-enabled #player-legacy").css("width", "100%");
					$("#player, #player-legacy, .site-left-aligned.guide-enabled #player-legacy").css("padding-left", "0px");
					$("#player, #player-legacy, .site-left-aligned.guide-enabled #player-legacy").css("padding-right", "0px");
				}
	...
	toge.addEventListener('click', function () {
		GM_config.open()
	}, false);
	...
	if (GM_config.get("expande2")) {
		...
		$("#watch-description").on('click', function () {
			timer = setTimeout(function () {
					$("#watch-description-content").css("height", "auto");
					$("#watch-description-extras").css("display", "block");
					$("#watch-description-content.extra-info").css("max-height", "auto");
				}, 0);
		});
	...
	if (GM_config.get("collaps2")) {
		...
		$("#watch7-sidebar").on('mouseenter', function () {
			timer3 = setTimeout(function () {
					$("#watch7-sidebar").css("height", "auto");
					$("#watch7-sidebar").css("opacity", "0.9");
				}, 700);
		});
	...
	if (GM_config.get("collapse")) {
		...
		$("#watch-discussion").on('click', function () {
			timer2 = setTimeout(function () {
					...
					$("#watch-discussion").css("max-height", "none");
...
function attachBar(videoThumb, published, views, likes, dislikes) {
	...
	if (total > 0) {
		...
		if (waitStyle != "") {
			likesDiv.setAttribute("class", "video-extras-sparkbar-likes likesBar waitingBar");
		}
		...
		if (likeStrengthBar > 0) {
			ratingDiv.appendChild(likeStrengthDiv);
		}
		if (purpleBar > 0) {
			ratingDiv.appendChild(purpleDiv);
		}
		if (likesBar > 0) {
			ratingDiv.appendChild(likesDiv);
		}
		if ((likeStrengthBar + purpleBar) > 0) {
			ratingDiv.appendChild(lsLinesDiv);
		}
